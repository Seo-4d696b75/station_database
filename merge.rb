...
class DataParser
	...
	def merge(data_path="merge.json")
		...
		lines.select do |e|
			...
			if e.key?("code")
				...
				if e.key?("remove") && e["remove"]
					if @line_map.delete(code) && @line.delete(line)
						puts "Log > remove line entry : " + line.to_json
					else
						puts "Error > fail to remove line entry"
						return
					end
				...
				if @line_name_set.add?(name)
					add = !e.key?("code")
					if !add then @line_set.add(e["code"]) end
					line = LineItem.new(e, (add ? get_line_code(name) : nil))
					@line.push(line)
					@line_map[line.code] = line
					##追加される際の新コードを保存
					if add && !(str = add_code(str, ("\"line\":\"%s\"" % name), line.code))
		...
		stations.select do |e|
			...
			if e.key?("code")
				...
				##エントリから消去
				if e.key?("remove") && e["remove"]
					if @station_map.delete(code) && @station.delete(s)
						puts "Log > remove station entry ; " + s.to_json
					else
						puts "Error > fail to remove station entry"
						return
					end
		...
		lines.each do |e|
			if e.key?("add_station") || e.key?("remove_station")
				...
				if line = remove ? removed_line_map[name] : @line_name_map[name]
					if e.key?("add_station") 
						if remove
							puts "Error > adding station item to removed line. " + line.to_json
							return
						end
						puts "Log > add station item to line : " + line.to_json
						e["add_station"].each do |name|
							if add_station_item(line,name,false)
								print name + ", "
							else return end
						end
					...
					if e.key?("remove_station")
						...
						puts "    this line has removed from list yet." if remove
	...
	def add_code(data,key,code)
		...
		if m = regex.match(data)
			return ("%s\"code\":%d,%s" % [m[1], code, m[2]])
		else
			return nil
		end
	end
