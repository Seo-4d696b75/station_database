...
| [id](#id)                        | `integer` | Required | non-null | [路線詳細オブジェクト](line_detail-properties-登録駅リスト-駅オブジェクト路線登録-properties-駅id.md "undefined#/properties/station_list/items/properties/id")                   |
...
| [attr](#attr)                    | `string`  | Required | non-null | [路線詳細オブジェクト](line_detail-properties-登録駅リスト-駅オブジェクト路線登録-properties-駅の属性.md "undefined#/properties/station_list/items/properties/attr")                |
...
駅の識別子. 駅コードとは異なり、別バージョンのデータセット間でも一貫性を保証します（駅メモ実装における「同じ」駅のIDは異なるデータセットでも同じIDになります）. IDは駅メモ公式Webサイトの「駅の思い出」ページのURL <https://ekimemo.com/database/station/{id}/activity> に対応しています. 独自追加の廃駅のIDは10000番台の連番を使用しています.
