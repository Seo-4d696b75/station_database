...
|名前|値の型|値の内容|欠損の有無|stations.json|out/stations.json|out/data.json|    
|:-:|:-:|:--|:--|:-:|:-:|:-:|  
|code|整数値|各駅に固有な値|なし|〇|〇|〇|  
|station|文字列|駅の名称|なし|〇|〇|〇|  
|closed|真偽値|廃駅であるか|あり、falseの場合は省略|〇|〇|〇|  
|phonetic|文字列|駅名称のひらがな表記|なし|×|〇|〇|  
|prefecture|整数値|駅の所在都道府県|なし|×|〇|〇|  
|lon|浮動小数|駅の所在経度|なし|〇|〇|〇|  
|lat|浮動小数|駅の所在緯度|なし|〇|〇|〇|  
|lines|配列（整数値の要素）|駅の登録されている路線コードのリスト|なし|〇|〇|〇|  
|next|配列（整数値の要素）|隣接駅のコードのリスト|なし|×|〇|〇|  
|voronoi|Object|駅のボロノイ分割領域を表す|なし|×|〇|〇|  
...
* "next"では駅座標を母点としてドロネー三角分割したときドロネー辺で結ばれる点どうしに隣接関係を定義する
* "voronoi"オブジェクトの内容は以下の通りボロノイ境界の多角形を表す
	* "pivot"は浮動小数の要素２つをもつ配列であり、```[lng,lat]```の順で多角形の一点を表す
	* "points"は浮動小数の配列であり、```[lng,lat,lng,lat, ...]```の形式で"pivot"が示す座標点からはじまり前の座標との差分x10^6を順に表す
	* "points"の最後の点と"pivot"の点も繋がっており一つのポリゴンを成す

### 路線データ
以下の属性をもつ  

|名前|値の型|値の内容|欠損の有無|lines.json|out/lines.json|out/data.json|  
|:-:|:-:|:--|:--|:-:|:-:|:-:|  
|code|整数値|各路線に固有な値|なし|〇|〇|〇|  
|line|文字列|路線の名称|なし|〇|〇|〇|  
|closed|真偽値|廃線であるか|あり、falseの場合は省略|〇|〇|〇|  
|phonetic|文字列|路線名称のひらがな表記|なし|×|〇|〇|  
|symbol|文字列|路線記号|あり|×|〇|〇|  
|color|文字列|路線カラー|あり|×|〇|〇|  
|size|整数値|路線に登録された駅数|なし|〇|×|×|  
|list_size|整数値|路線に登録された駅数|なし|×|〇|〇|  
|station_list|Object|登録されている駅情報のリスト|廃線の場合欠損あり|×|×|〇|  
|point_list|配列|路線ポリラインのリスト|廃線の場合欠損あり|×|×|〇|  
...
* "station_list"には駅メモのデータと同じ順序で路線に登録された駅を要素にもつ  
	* "code"は駅コード（欠損なし）
	* "numbering"は駅ナンバリング情報で欠損もある.駅によっては複数のナンバリングを持つ場合があるので配列である.各要素の詳細は以下の通り

|名前|値の型|値の内容|欠損の有無|  
|:-:|:-:|:--|:--|  
|index|整数値|ナンバリングのうち数値部分|なし|  
|symbol|文字列|ナンバリングのうち非数値部分|あり、路線の"symbol"と同じ場合に省略される|  
|color|文字列|RGB表現で"#[0-9A-F]{6}"|あり、路線の"color"と同じ場合に省略される|  
